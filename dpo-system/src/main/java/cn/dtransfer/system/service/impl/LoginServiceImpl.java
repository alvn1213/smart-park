...
import cn.dtransfer.common.exception.user.CaptchaException;
import cn.dtransfer.common.exception.user.UserBlockedException;
import cn.dtransfer.common.exception.user.UserNotExistsException;
import cn.dtransfer.common.exception.user.UserPasswordNotMatchException;
...
public class LoginServiceImpl implements ILoginService {
     ...
     */
    @Override
    public User login(String username, String password, User.Type type) {
        // 用户名或密码为空 错误
        ...
        // 查询用户信息
        User user = userService.selectUserByLoginName(username, type);
    ...
    public User loginCaptcha(String mobile, String captcha) {
        ...
        if (user == null) {
            PublishFactory.recordLoginInfo(mobile, Constants.LOGIN_FAIL, MessageUtils.message("user.not.exists"));
            throw new UserNotExistsException();
        }
        if (UserStatus.DISABLE.getCode().equals(user.getStatus())) {
            PublishFactory.recordLoginInfo(mobile, Constants.LOGIN_FAIL, MessageUtils.message("user.blocked"));
            throw new UserBlockedException();
        }
