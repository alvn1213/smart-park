      ...
      <a-col :md="24" :lg="16">
        ...
        <a-form @submit="handleSubmit" :form="form" layout="vertical">
          ...
          <a-form-item label="用户名">
            <a-input placeholder="用户名" v-decorator="['nickname', {rules: [{ required: true, message: '请输入用户名' }]}]"/>
          </a-form-item>
          <!-- <a-form-item label="登录名" >
            <a-input placeholder="登录名" v-decorator="['userName', {rules: [{ required: true, message: '请输入登录名' }]}]" disabled/>
          </a-form-item> -->
          <a-form-item label="手机" >
            <a-input placeholder="手机" v-decorator="['mobile', {rules: [{ required: true, message: '请选录入手机' }]}]" disabled/>
          </a-form-item>
          <a-form-item label="性别">
            <a-radio-group @change="onChange" v-decorator="['gender']" >
...
import { updataProfile } from '@/api/system'
  ...
  methods: {
    ...
    // 用户信息
      getInfo().then(record => {
        ...
        this.option.img = (this.mdl.avatar == "" || this.mdl.avatar == null) ? require("@/assets/images/profile.jpg") :  process.env.VUE_APP_API_BASE_URL + this.mdl.avatar;
        console.log('this.option.img', this.option.img)
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.mdl, 'id', 'nickname', 'userName', 'gender', 'post', 'mobile', 'email'))
        })
      })
    },
    setAvatar (url) {
