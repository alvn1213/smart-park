...
export default {
  ...
  data () {
    return {
      ...
      statusOptions: [],
      jobGroupMap,
      addEnable: checkPermission('monitor:job:add'),
      editEnabel: checkPermission('monitor:job:edit'),
      removeEnable: checkPermission('monitor:job:remove')
    }
  },
  filters: {
  ...
  methods: {
    ...
    onChangeStatus (record) {
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          changStatus(pick(record, 'id', 'status')).then(res => {
            if (res.code === 0) {
    ...
    handleRun (record) {
      ...
      this.$confirm({
        ...
        cancelText: '否',
        onOk () {
          runJob(pick(record, 'id', 'jobGroup')).then(res => {
            if (res.code === 0) {
              _this.$message.success('操作成功')
            } else {
              _this.$message.error(res.msg)
