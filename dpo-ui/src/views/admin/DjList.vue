...
import { getDjList, delDj, changMarketable } from '@/api/admin/dj'
...
export default {
  ...
  data () {
    return {
      ...
      loadData: parameter => {
        return getDjList(Object.assign(parameter, this.queryParam))
      },
      selectedRowKeys: [],
      selectedRows: [],
      addEnable: checkPermission('admin:dj:add'),
      editEnabel: checkPermission('admin:dj:edit'),
      removeEnable: checkPermission('admin:dj:remove')
    }
  },
  filters: {
  },
  ...
  methods: {
    onChangeStatus (record) {
      ...
      this.$confirm({
        title: '警告',
        content: '确认要' + text + '【' + record.djName + '】' + '吗?',
        okText: '是',
        okType: 'warning',
        cancelText: '否',
        onOk () {
    ...
    handleEdit (id) {
      this.$router.push({ name: 'djEdit', query: { id: id } })
    },
    handleOk () {
