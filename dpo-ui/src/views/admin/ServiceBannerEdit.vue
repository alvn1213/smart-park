...
import { getServiceBanner, saveServiceBanner } from '@/api/admin/serviceBanner'
...
export default {
  ...
  methods: {
    handleInit () {
      ...
      if (id) {
        getServiceBanner(id).then(record => {
          ...
          this.$nextTick(() => {
            this.form.setFieldsValue(pick(this.mdl, 'id', 'bannerName', 'uri', 'bannerDesc', 'bannerImg', 'hits', 'isMarketable', 'isTop'))
          })
        })
      }
    },
    ...
    handleSubmit (e) {
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          saveServiceBanner(values).then(res => {
            if (res.code === 0) {
    ...
    rollback () {
      this.$router.push('/admin/service/serviceBanner')
    }
  }
}
