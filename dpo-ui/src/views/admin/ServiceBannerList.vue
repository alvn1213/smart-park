  ...
  <a-card :bordered="false">
        ...
        <a-row :gutter="48">
          <a-col :md="5" :sm="15">
            <a-form-item label="banner名称">
              <a-input placeholder="请输入banner名称" v-model="queryParam.bannerName"/>
            </a-form-item>
          </a-col>
          <a-col :md="8" :sm="24">
    ...
    >
      ...
      <span slot="isTop" slot-scope="text, record">
        <a-switch :checked="record.isTop==true" @change="onChangeTopStatus(record)"/>
      </span>
...
import { getServiceBannerList, delServiceBanner, changMarketable, changTop } from '@/api/admin/serviceBanner'
...
export default {
  ...
  data () {
    return {
      ...
      loadData: parameter => {
        return getServiceBannerList(Object.assign(parameter, this.queryParam))
      },
      selectedRowKeys: [],
      selectedRows: [],
      addEnable: checkPermission('admin:ServiceBanner:add'),
      editEnabel: checkPermission('admin:ServiceBanner:edit'),
      removeEnable: checkPermission('admin:ServiceBanner:remove')
    }
  },
  filters: {
  },
  ...
  methods: {
    ...
    onChangeTopStatus (record) {
      const _this = this
      console.log('record : ', record)
      const text = record.isTop === false ? '置顶' : '取消置顶'
      this.$confirm({
        title: '警告',
        content: '确认要' + text + '【' + record.bannerName + '】' + '吗?',
        okText: '是',
        ...
        onOk () {
          ...
          changTop(pick(record, 'id', 'isTop')).then(res => {
            if (res.code === 0) {
              _this.$message.success('操作成功')
              _this.$refs.table.refresh(true)
            } else {
    ...
    handleEdit (id) {
      this.$router.push({ name: 'serviceBannerEdit', query: { id: id } })
    },
    handleOk () {
    ...
    delByIds (ids) {
      delServiceBanner({ ids: ids.join(',') }).then(res => {
        if (res.code === 0) {
