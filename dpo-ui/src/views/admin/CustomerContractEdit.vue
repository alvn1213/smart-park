  ...
  <a-card :body-style="{ padding: '24px 32px' }" :bordered="false">
    ...
    <a-form @submit="handleSubmit" :form="form" :label-col="{ span: 4 }" :wrapper-col="{ span: 6 }">
      ...
      <div v-if="step == 0">
        <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="合同名称">
          <a-input
            style="width: 60%"
            placeholder="请输入合同名称"
            v-decorator="['name', { rules: [{ required: true, message: '请输入合同名称' }] }]"
          />
        </a-form-item>
        <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="租金合同号">
          <a-input
            style="width: 60%"
            placeholder="请输入租金合同号"
            v-decorator="['sn', { rules: [{ required: true, message: '请输入租金合同号' }] }]"
          />
        </a-form-item>
        <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="物管合同号">
          <a-input
            style="width: 60%"
            placeholder="请输入物管合同号"
            v-decorator="['pmSn', { rules: [{ required: true, message: '请输入物管合同号' }] }]"
          />
        </a-form-item>
        <!-- <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="合同类型">
          <a-radio-group name="type" :default-value="formData.type" @change="typeChange">
            <a-radio :value="0"> 整租合同 </a-radio>
            <a-radio :value="1"> 工位合同 </a-radio>
          </a-radio-group>
        </a-form-item> -->
        <!-- <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="发票类型">
          <a-select v-decorator="['invoiceType', {initialValue:'',rules: [{ required: true, message: '请选择发票类型' }]}]">
            <a-select-option value="0">普通发票</a-select-option>
            <a-select-option value="1">专用发票</a-select-option>
          </a-select>
        </a-form-item> -->
        ...
        <!-- <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="客户名称">
          <a-auto-complete
            ...
            @search="onSearch">
            <template slot="dataSource">
              <a-select-option v-for="item in dataSource" :key="item.id" :value="item.supplierName"> {{ item.supplierName }}</a-select-option>
            </template>
          </a-auto-complete>
        </a-form-item> -->
      ...
      <div v-if="step == 1">
        ...
        <!-- <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="首期收款日：">
        ...
        <div style="border: 1px solid #dddddd; width: 70%; margin: 0 auto">
          ...
          >
            ...
            <span style="margin-left: 20px">租赁面积：<span>{{ formData.rentArea }} 平米</span></span>
          </div>
        ...
        <div style="width:100%;text-align:center;margin-top:30px;">
          <a-button key="buy" @click="navBack">返回</a-button>
          <a-button style="margin-left: 8px" html-type="submit" type="primary">保存</a-button>
      ...
      if (this.id) {
        getCustomerContract({ id: this.id }).then((record) => {
          ...
          if (this.step === 0) {
            ...
            this.formData.rentArea = rentArea.toFixed(2)
            ...
            this.$nextTick(() => {
              ...
              this.formData.customerContractOtherBills = record.customerContractOtherBills
              ...
              if (this.default === true) {
                this.formData.rentPriceUnit = '1'
                this.formData.managementFeeUnit = '1'
                this.getManagementFee(3)
                this.form.setFieldsValue({
                  rentPrice: '',
                  rentIncreaseDay: '',
                  rentIncreaseMonth: '',
