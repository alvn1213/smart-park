  ...
  <a-card :body-style="{padding: '24px 32px'}" :bordered="false">
    <a-form @submit="handleSubmit" :form="form" :label-col="{ span: 4 }" :wrapper-col="{ span: 6 }">
      ...
      <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="banner名称" extra="不要超过6个字符">
        <a-input style="width: 60%" placeholder="请输入banner名称" v-decorator="['name', {rules: [{ required: true, message: '请输入banner名称' }]}]"/>
      </a-form-item>
      ...
      <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="落地页URI">
        <a-input style="width: 60%" placeholder="落地页URI" v-decorator="['uri', {rules: [{ required: true, message: '请输入落地页URI' }]}]"/>
      </a-form-item>
...
import { getPolicyBanner, savePolicyBanner } from '@/api/admin/policyBanner'
...
export default {
  ...
  methods: {
    handleInit () {
      ...
      if (id) {
        getPolicyBanner(id).then(record => {
          ...
          this.$nextTick(() => {
            this.form.setFieldsValue(pick(this.mdl, 'id', 'name', 'uri', 'bannerDesc', 'bannerImg', 'hits', 'isMarketable', 'isTop'))
          })
        })
      }
    },
    ...
    rollback () {
      this.$router.push('/admin/policy/policyBanner')
    }
  }
}
