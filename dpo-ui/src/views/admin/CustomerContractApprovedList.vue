  ...
  <a-card :bordered="false">
    ...
    >
      ...
      <span slot="action" slot-scope="text, record">
        <a @click="showModal(2, [record.id])">审批</a>
        <a-divider type="vertical" />
<!--        <a @click="billEdit(record)">账单</a>-->
      </span>
    </s-table>
...
export default {
  ...
  data () {
    return {
      ...
      loadData: parameter => {
        this.queryParam.status = 'APPROVING'
  ...
  methods: {
    ...
    handleDetail (id) {
      // this.$router.push({ name: 'customerContactsDetailModal', query: { id: id, url: 'contractApproved' } })
