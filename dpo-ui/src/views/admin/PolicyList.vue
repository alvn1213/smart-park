<template>
  <a-card :bordered="false">
    ...
    <dj-modal ref="modal" @ok="handleOk"/>
  </a-card>
</template>

<script>
import { STable } from '@/components'
import { getPolicyList, delPolicy, changMarketable } from '@/api/admin/policy'
...
export default {
  ...
  data () {
    return {
      ...
      loadData: parameter => {
        return getPolicyList(Object.assign(parameter, this.queryParam))
      },
      selectedRowKeys: [],
      selectedRows: [],
      addEnable: checkPermission('admin:policy:add'),
      editEnabel: checkPermission('admin:policy:edit'),
      removeEnable: checkPermission('admin:policy:remove')
    }
  },
  filters: {
  },
  ...
  methods: {
    ...
    handleEdit (id) {
      this.$router.push({ name: 'policyEdit', query: { id: id } })
    },
    handleOk () {
    ...
    delByIds (ids) {
      delPolicy({ ids: ids.join(',') }).then(res => {
        if (res.code === 0) {
