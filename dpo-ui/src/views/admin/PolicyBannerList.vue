...
import { getPolicyBannerList, delPolicyBanner, changMarketable } from '@/api/admin/policyBanner'
...
export default {
  ...
  data () {
    return {
      ...
      loadData: parameter => {
        return getPolicyBannerList(Object.assign(parameter, this.queryParam))
      },
      selectedRowKeys: [],
      selectedRows: [],
      addEnable: checkPermission('admin:policyBanner:add'),
      editEnabel: checkPermission('admin:policyBanner:edit'),
      removeEnable: checkPermission('admin:policyBanner:remove')
    }
  },
  filters: {
  },
  ...
  methods: {
    ...
    handleEdit (id) {
      this.$router.push({ name: 'policyBannerEdit', query: { id: id } })
    },
    handleOk () {
    ...
    delByIds (ids) {
      delPolicyBanner({ ids: ids.join(',') }).then(res => {
        if (res.code === 0) {
